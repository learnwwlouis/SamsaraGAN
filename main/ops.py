...
def c7s1_k(input, k, reuse=False, norm='instance', activation='relu', is_training=True, name='c7s1_k'):
  ...
  with tf.variable_scope(name, reuse=reuse):
    ...
    output = _norm(conv, is_training, norm)
...
def last_conv(input, num_filter, reuse=False, use_sigmoid=False, name=None):
  ...
  with tf.variable_scope(name, reuse=reuse):
    weights = _weights("weights",
      shape=[4, 4, input.get_shape()[3], num_filter])
    biases = _biases("biases", [num_filter])
